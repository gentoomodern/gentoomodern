...
import os, docker
from .gentoomodern_common import desired_profile_path, profiles_amd64
from .stage_exists import stage_exists


def set_desired_profile(arch, profile):
    if profile not in profiles_amd64 or not (stage_exists(arch, profile, 'gentoomodern/builder', False) or stage_exists(arch, profile, '', True)):
